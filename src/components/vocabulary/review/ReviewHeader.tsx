...
interface ReviewHeaderProps {
  ...
  railOpen: boolean;
  onToggleRail: () => void;
  /** Where the exit link returns to — the deck this session came from. */
  exitTo: string;
  title: string;
  /** Exit wording. Browsing a deck is not reviewing it, so the label follows the mode. */
  exitLabel?: string;
}

/** Quiet session header: exit on the left, centred title, count + minimal progress on the right. */
export function ReviewHeader({ position, total, railOpen, onToggleRail, exitTo, title, exitLabel = 'Exit review' }: ReviewHeaderProps) {
