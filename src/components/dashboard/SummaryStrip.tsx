...
export function SummaryStrip({
  ...
  return (
    ...
    // Stays at 3xl: the design system steps the hero and the stat panels up from the 2xl the regular
    // section cards below sit at.
    <div className="grid grid-cols-1 divide-y divide-slate-200 rounded-3xl border border-slate-200 bg-white px-2 py-2 shadow-sm lg:grid-cols-3 lg:divide-y-0 dark:divide-hairline dark:border-hairline dark:bg-ink-900">
      <SummaryMetric
      ...
      <SummaryMetric
        ...
        value={`${studyDays} / ${weeklyGoalDays}`}
        unit="days"
        label="Weekly goal"
      />
    </div>
...
function SummaryMetric({
  ...
  return (
    ...
    <div className="relative flex items-center gap-4 px-5 py-4 lg:[&:not(:first-child)]:before:absolute lg:[&:not(:first-child)]:before:inset-y-4 lg:[&:not(:first-child)]:before:left-0 lg:[&:not(:first-child)]:before:w-px lg:[&:not(:first-child)]:before:bg-slate-200 dark:lg:[&:not(:first-child)]:before:bg-hairline">
