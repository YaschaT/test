...
export function ExamResults({ level, questions, answers, isNewBest, onRetake, onExit }: ExamResultsProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Verdict and score sit beside the breakdown on a wide screen, stacked below it on a narrow one. */}
      <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,22rem)_minmax(0,1fr)]">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-sm lg:sticky lg:top-8 dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mx-auto mt-1.5 max-w-md text-sm text-slate-500 dark:text-slate-400">
            {subtitle.en}
            <span className="mt-0.5 block text-xs text-slate-400 dark:text-slate-500">{subtitle.nl}</span>
          </p>
          ...
          <div className="mt-6 flex flex-col gap-2">
            <PrimaryButton onClick={onRetake} className="w-full !py-3">
              <RotateCcw size={16} /> Retake exam
            </PrimaryButton>
            <button onClick={onExit} className="rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
              Back to overview
            </button>
          </div>
        </div>

        {/* Breakdown column: the sectional scores and the per-question review, so the space beside the
            verdict keeps filling as you open things up. */}
        <div className="space-y-4">
